    ...
    <div className={`${switchThemeCss.theme_container} ${ModalStyle.theme_container}`}>
      <div>
        {theme ? (<>
          {props.text === "Light" ? (
              <div>
                <MdLightMode size={20} onClick={() => setTheme(false)}/>
              </div>
            ) : (
              <div>
                <MdDarkMode size={20} onClick={() => setTheme(false)} />
              </div>
            )}
          </>
        ) : (
          <>
            {props.text === "Light" ? (
              <div>
                <MdLightMode size={20} onClick={() => setTheme(true)}/>
              </div>
            ) : (
              <div>
                <MdDarkMode size={20} onClick={() => setTheme(true)} />
              </div>
            )}
          </>
        )}
